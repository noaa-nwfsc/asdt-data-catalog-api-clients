...
POSSIBLE_TARGS="develop master main test jenkins_dev"
declare -a TARGET_BRANCHES=()
REMOTE_BRANCHES="$(git branch -r|grep -v '\->'|sed -e 's+^\s*++' -e 's+origin/++'|tr '\n' ' ')"
for B in $REMOTE_BRANCHES ;do
	if [[ " $POSSIBLE_TARGS " =~ " $B " ]]; then
		TARGET_BRANCHES+=($B)
	fi
done
...
usage() {
	...
	echo "                   Non-existant members of the list are skipped."
...
# Check for Modified/Added (Not Committed) Files
...
# Check for Untracked Files:  grep '^??' isolates strictly untracked files
...
for branch in "${TARGET_BRANCHES[@]}"; do
	...
	if ! git ls-remote --exit-code --heads "$REPO" >&  ;then
		echo "  Skipping $branch, not present."
	fi
	if ! git ls-remote --exit-code --heads "$(repo)" "master1"
