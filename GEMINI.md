...
## 1. Overview
This repo provides "Elite" SDKs for the NWFSC Data Catalog in both **Python** and **R**. It is specifically architected to run in two environments:
1.  **Standard Desktop**: Full-featured environments with native networking (`urllib3`, `curl`).
2.  **WebAssembly (WASM)**: Browser-based execution via **Pyodide** (Python) and **WebR** (R), powering the React-based "API Coding Lab."
...
- **`scripts/python/build_tools.py`**: The core build orchestrator. This script is responsible for:
    ...
    - **Code Generation**: It uses the bootstrapped JRE to run the `openapi-generator-cli.jar` and generate the base Python and R client code from the `openapi/openapi-public.json` specification.
    - **Packaging & Testing**: It handles the language-specific packaging steps (e.g., running `uv build` for Python, `R CMD build` for R) and executes the test suites in isolated environments.
- **`scripts/python/version_manager.py`**: This script provides automated semantic versioning. Before a build runs, it calculates the hash of the OpenAPI spec. If the hash has changed since the last build, it automatically increments the patch version of both the Python and R packages.
...
Both languages follow a consistent naming convention to provide a "Data Science First" experience:
- `get_<resource>()`: Low-level call returning Pydantic models (Py) or Lists (R).
- `read_<resource>()`: Returns a **Pandas DataFrame** or **R Tibble**.
- `fetch_all_<resource>()`: Implements auto-pagination to pull the entire dataset.
...
- **Polyglot Consistency**: Ensure new API features are implemented in both `scripts/python/factory.py` (for Python wrappers) and `scripts/r/generate_r_wrappers.R` (for R wrappers).
