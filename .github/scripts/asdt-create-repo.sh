...
usage(){
	...
	echo -e "   -f|--force By default the script will not overwrite existing "
	echo -e "            repositories.  If this flag is set, it will."
	...
	echo -e "    --visiblity: The visibility level of the created repo."
	echo -e "                  Must be internal,private, or public. "
	echo -e "                  Can be set with environment variable VISIBILITY"
	echo -e "                  Default: $_DEF"
...
while [ $# -ne 0 ]; do
   	  case "$1" in
		  ...
		  -v|--verbose)
			  (( VERBOSE ++ ))
			  if [ $VERBOSE -gt 2 ];then set -xv;fi
			  shift		  
			  ;;
...
if [ "$VISIBILITY" = "" ];then export VISIBILITY="$VISIBILITY_DEF";fi
if [ "$VISIBILITY" != "internal" \
				   -a "$VISIBILITY" != "private" \
				   -a "$VISIBILITY" != "public" ];then
	echo -e "Aborting: value of visiblity ($VISIBILITY) is unsupported."
	usage 2>&1
	exit
fi
set +xv
...
# 1.a Check if template exists
if [ $QUIET -eq 0 ];then echo "Checking if template repo ($SOURCE) is present.";fi
CMD="verify_repo $SOURCE ${LOG_OUT}"
if ! LOG="$(eval "$CMD")"  ;then abort "Cannot verify template repo ($SOURCE)," "$LOG";fi

# 1.b Check it TARGET is not present. If exists: Default abort, Forced, delete
if [ $QUIET -eq 0 ];then echo "Checking if target ($TARGET) is available";fi
CMD="verify_repo $TARGET ${LOG_OUT}"
if LOG="$(eval $CMD)"  ;then
	if [ $FORCE -eq 0 ];then abort "Target repo ($TARGET) exists: will not overwrite" ;fi
	if [ $QUIET -eq 0 ];then echo "Force flag set: Will overwrite $TARGET";fi
	if ! LOG="$(delete_repo "$TARGET")";then abort "Overwriting $TARGET failed." "$LOG";fi
fi

#2. Create TARGET from SOURCE template
MSG="Creating $TARGET from template $SOURCE"
if [ $QUIET -eq 0 ];then echo "Creating $TARGET from template $SOURCE...";fi
CMD="create_from_template '$SOURCE' '$TARGET' $LOG_OUT"
if ! LOG="$(eval $CMD )";then abort "Failed: $MSG" "$LOG" 1>&2
	exit 1
fi
	 
# 3. Sync Permissions for each team
...
# 4. Sync Repository Branch Rulesets
...
if [ $QUIET -eq 0 ];then echo "Templated repository creation completed.";fi
