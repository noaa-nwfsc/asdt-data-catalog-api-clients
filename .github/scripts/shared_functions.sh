...
create_from_template(){
	...
	if [ $# -ne 2 ];then
		...
		echo "         usage: create_from_templates SOURCE NEW_REPO" 2>&1
		return 1
	fi
	TEMPLATE_REPO="$1"
	NEW_REPO="$2"
	...
	if [ "$VISIBILITY" = "" ];then export VISIBILITY=internal;fi
	CMD="gh repo create '$NEW_REPO' --template '$TEMPLATE_REPO' --$VISIBILITY --include-all-branches"
	if [ $VERBOSE -ne 0 ];then
...
copy_repo_files(){
	...
	done < <(gh api /repos/noaa-nwfsc/asdt-template/git/trees/master?recursive=true | jq -r '.tree[] | select(.type == "blob") | .path')
	...
	if ! LOG="$(eval "$CMD" 2>&1 )" ;then
			echo -e "\tcopy_repo_files: Failed  target repo ($TARG_REPO) clone using:" 1>&2
			echo -e "\tCMD: $CMD\n\tError messages: " 1>&2
			echo -e "$LOG"|sed 's+^+\t\t+' 
			popd >& /dev/null
			return 1
	...
	if ! LOG="$(eval "$CMD" 2>&1 )" ;then
			echo -e "\tcopy_repo_files: Failed  fetch of ($TARG_REPO) using:" 1>&2
			echo -e "\tCMD: $CMD\n\tError messages: " 1>&2
			echo -e "$LOG"|sed 's+^+\t\t+' 
			popd >& /dev/null
			return 1
